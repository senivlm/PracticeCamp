...
*/
        public Tensor(int dimention)
        {
            ...
            _tensorItems = new List<TensorItem<T>>();
        }
        public Tensor(int dimention, IEnumerable<TensorItem<T>> tensorItems)
        {
            _dimention = dimention;
            _tensorItems = new List<TensorItem<T>>(tensorItems.Where(item => item.Dimention == _dimention));
        }
        ...
        public static Tensor<T> operator +(Tensor<T> a, Tensor<T> b)
        {
            ...
            Tensor<T> tensor = new Tensor<T>(a._dimention, a._tensorItems);
